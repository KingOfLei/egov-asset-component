...
define(["app/core/app-base","bs-plugin/Env","bs-plugin/bs/GrPrint",
        "bs-plugin/bs/BsNetCtl",
	"bs-plugin/bs/GrPrintViewer","bs-plugin/bs/GrPrintDesigner","bs-plugin/bs/BsGridPrint"],
	function(Base,Env,GrPtrint,BsNetCtl,GrPrintViewer,GrPrintDesigner,BsGridPrint){
	BsReport=Base.extend({
		propsInAttrs: ['downLoadUrl','queryTempleteUrl', 'queryTemplistUrl', 'printTemplateUrl','cookies',"gridPrintTemplateurl","subReportCofnig"],
		downLoadUrl: '',
		queryTempleteUrl: '',
		queryTemplistUrl :'',
		printTemplateUrl:'',
		...
		/*//打印固定资产报废处置表
		printMultipleReport:function(loadReportURL,dataUrl, isPreview, conf,callBack) {
			//加载报表模板
			this.grPtrint.LoadFromURL(loadReportURL);
			var _self=this;
			//设置子报表
			//报废处置明细表
			var bfczmxb = _self.grPtrint.ControlByName("BFCZMXB").AsSubReport.Report;
			//报废处置汇总表
			var bfczhzb = _self.grPtrint.ControlByName("BFCZHZB").AsSubReport.Report;
		...
		_showTemplatePanel :function () {
			...
			html += "<table id=\"_print_grid\" ></table> ";
			html += "</div> ";
			//html += "</div> ";
			...
			//html += "<a id=\"btn_print_set\" class=\"app-button l-btn l-btn-small\" tips=\"设置\"></a>";
			...
			html += "<a id=\"btn_print_cancel\" class=\"app-button l-btn l-btn-small\"><span class=\"l-btn-left\"><span class=\"l-btn-text\">关闭</span></span></a></div></div>";
//			html += "</div></div>";
		...
		showPrintTemplateDialog : function(moduleId,  
			...
			var options = {
					...
					afterShow:function(){
						...
						$A('#_print_grid').grid({
							columns: [[{
								field: 'moduleItemCode',
								title: '编码'
							},{
								field: 'moduleItemName',
								title: '名称'
							}]],
							url: 'egov/asset/template/queryTemplateDetailPage.do',
							autoLoad: false,
							title:'打印模板信息',
							radiobox:true,
