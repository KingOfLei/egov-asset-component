 ...
 *</p>
 */
public class TaskTriggerMiddle implements org.activiti.engine.delegate.TaskListener, Serializable {
	private static Logger logger = LoggerFactory.getLogger(TaskTriggerMiddle.class);
	/**
	 * 
	 */
...
//		aas.addAlarm(task);
		try {
			aas.updateMiddle(task, alarm.getZone(), alarm.getProcessDesc(), alarm.getIsAlarm());
		} catch (Exception e) {
			logger.error("监听类操作数据库报错_TaskTriggerMiddle_updateMiddle__" + e.getMessage());
			throw new BusinessException("728","监听类操作数据库报错_TaskTriggerMiddle_updateMiddle__" + e.getMessage());
		}
		//查找是否存在
		//存在：什么都不做
