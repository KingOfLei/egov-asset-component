...
define(['app/core/app-base', 'app/core/app-jquery', 'app/core/app-core', 'app/core/app-options', 'app/widgets/app-progressbar',
	...
	var Upload = Base.extend({
		...
		_initFileElement: function(fileInfos){
			...
			/**
			 * 扩展修改-2017-12-15
			 * 增加参数 titInfo, 用于修改title显示值
			 */
			var titInfo = fileInfos.name + '（'+ fileInfos.createDate2 +'）';
			var $file = $('<div class="file-item" title="' + titInfo + '">' +
							'<div ' + getFileIconCls(fileInfos.ext) + '></div>' +
							nameHtml(fileInfos.name, titInfo) + 
	 ...
	 * 增加参数 titInfo, 用于修改title显示值
	 */
	function nameHtml(fileName, titInfo){
		var namegbLen = fileName.gblen()
			,firstLen = 12;
		if(namegbLen <= firstLen){
			return '<div class="file-name" title="' + titInfo + '">' + fileName + '</div>'; 
		}
		var name1 = ''
			,name2 = ''
		if(namegbLen <= 20){
		...
		return ('<div class="file-name" title="' + titInfo + '">' + name1 + '</div>'
				+ '<div class="file-name" title="' + titInfo + '">' + name2 + '</div>'); 
	}
	/**
	 * 本地进度（假进度）
	 * @param progressbar
