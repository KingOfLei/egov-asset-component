     ...
     var TabsExtension = ExtBase.extend({
         ...
         init:function(config){
             ...
             if(_self.tabsId){
                 ...
                 _self.$tabsHandler = $(_self.tabsId);
             }
             _self.extConfig(_self.defaultOption,_self);
		 ...
		 render: function () {
             ...
             if(CUtil.isNullStr(_self.tabsId)){
                 ...
                 _self.$tabsHandler.append(_self.tabs);
             }
            ...
            //定位方法只调用一次
             ...
             //重置tab-content 宽度
             _self.$component.find('.tab-content>div').width(_self.$component.width())
         },

		 bind: function () {
             var _self = this;
             _self.$component.on('tabSwitch',_self.tabSwitchEvent);
             ...
             if(_self.isFlow){
                 ...
                 //单击标签事件
                 _self.$tabsHandler.find('ul li a').on('click',function(e){
         ...
         adjustNavSel:function (target) {
             ...
             if(!_self.$tabsHandler.find('a[data-target$="#'+target+'"]').parent().is(':visible')){
                 return;
             }
             ...
             _self.$tabsHandler.find('a[data-target$="#'+target+'"]').each(function (index,item) {
                 var $item = $(item);
                 $item.parent().addClass('hover');
                 _self.$component.trigger('tabSwitch',[$item.parent(),$item.data('target')]);
             })
         ...
         hideTab:function (id) {
             var _self = this;
             _self.$tabsHandler.find('a[data-target$="#'+id+'"]').parent().hide();
             _self.$component.find('#'+id).hide();
         },
         showTab:function (id) {
             var _self = this;
             _self.$tabsHandler.find('a[data-target$="#'+id+'"]').parent().show();
             _self.$component.find('#'+id).show();
         }
	 });
